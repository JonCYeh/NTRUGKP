...
def mXsec_LLL_save(f, g, q, n, path):
    mX_sec = mXsec_basis(f, g, q, n)
    LLL_save(path, mX_sec)
...
for i in [-1]:#range(len(Ns)-1):
    ...
    for _ in tqdm(range(NUM)):
        ...
        if hinv:
            ...
            tl=LLL_pNTRU_len(h,n,q)#NTRU_len(h,n,d,q,p)
            temp_len[_]=tl
    ...
    if hinv:
        if p3:
            ...
            np.savetxt("%s/NTRU_LLL_n%i_q%i_p%i_d%i.txt"%(folder, n,q,p,d), optimal,comments="#sLlambda_1=%.2f"%(temp_len[indmax]))
            mXsec_LLL_save(optimal[2], optimal[3], q,n,  "%s/NTRU_mLLL_n%i_q%i_p%i_d%i.txt"%(folder, n,q,p,d))
        ...
        if p3:
            ...
            np.savetxt("%s/NTRU_LLL_n%i_q%i_p%i_d%i.txt"%(folder, n,q,p,d), optimal,comments="#sLlambda_1=%.2f"%(temp_len[indmax]))
            mXsec_LLL_save(optimal[1], optimal[2], q,n,  "%s/NTRU_mLLL_n%i_q%i_p%i_d%i.txt"%(folder, n,q,p,d))

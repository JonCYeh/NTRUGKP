...
for logq in range(2,12,2):
    ...
    for n in tqdm(NRANGE):
        ...
        for num in tqdm(range(NUM_samp)):
            ...
            temp_GS_pub+=[(n,min_gs)]


    folder = "Sampling_Cyclic"
    np.savetxt('%s/Cyclic_samples_HKZlambda_q%i.txt'%(folder,q), np.asarray(temp))
    np.savetxt('%s/NTRU_pub_samples_GS_pub_q%i.txt'%(folder,q), np.asarray(temp_GS_pub))
    ...
    g.show()
    g.save('%s/Cyclic_samples_HKZlambda_q%i.pdf'%(folder,q))


    gs_plot_p=scatter_plot(temp_GS_pub, alpha=0.5, markersize=10, axes_labels=[r'$n$', r'$min \| \xi_i^* \|$'])
    gs_plot_p.show()
    gs_plot_p.save('%s/Cyclic_samples_GSp_q%i.pdf'%(folder, q))

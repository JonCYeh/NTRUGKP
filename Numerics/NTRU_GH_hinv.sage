...
for logq in range(2,12,2):
    ...
    for n in tqdm(NRANGE):
        ...
        for num in tqdm(range(NUM_samp)):
            ...
            p=3
            try:
                h, hinv,f_vec, g_vec=mat_keypair(n,d,q,p)
            except:
                print("FAIL n,d,q,p=",n,d,q,p)
                break
    ...
    folder = "Sampling_hinv_p"

    #np.savetxt('%s/NTRU_samples_hinv_HKZlambda_q%i.txt'%(folder,q), np.asarray(temp))
    np.savetxt('%s/NTRU_lambda_samples_hinv_pub_HKZlambda_q%i.txt'%(folder,q), np.asarray(temp_plen))
    np.savetxt('%s/NTRU_pub_samples_hinv_GS_pub_q%i.txt'%(folder,q), np.asarray(temp_GS_pub))
    #np.savetxt('%s/NTRU_pub_samples_hinv_GS_sec_q%i.txt'%(folder,q), np.asarray(temp_GS_s))
    ...
    #g.show()
    #g.save('%s/NTRU_samples_hinv_HKZlambda_q%i.pdf'%(folder,q))
    ...
    gp.show()
    gp.save('%s/NTRU_samples_hinv_pub_HKZlambda_q%i.pdf'%(folder,q))

    gs_plot_p=scatter_plot(temp_GS_pub, alpha=0.5, markersize=10, axes_labels=[r'$n$', r'$min \| \xi_i^* \|$'])
    gs_plot_p.show()
    gs_plot_p.save('%s/NTRU_samples_hinv_GSp_q%i.pdf'%(folder, q))

    #gs_plot_s=scatter_plot(temp_GS_s, alpha=0.5, markersize=10, axes_labels=[r'$n$', r'$min \| \xi_i^* \|$'])
    #gs_plot_s.show()
    #gs_plot_p.save('%s/NTRU_samples_hinv_GSs_q%i.pdf'%(folder,q))
